...
// 他者評価：「入江 あおい」を評価
const PEER_NAME = '入江 あおい';
const OTHER_Q = [
  { stmt: '$は他者に対して自分の考えや目的を伝えながら、ともに協働して物事を進めることができる。',
    opts: [
      '$はあまり周囲に働きかけず、自分のペースで動くことが多い',
      '$は求められれば意見を伝えるが、積極的に周囲を動かすことは少ない',
      '$は自分の考えを伝えながら、周囲と協力して物事を進めることができる',
      '$は高い影響力で周囲を動かし、チーム全体の方向性をリードできる' ] },
  { stmt: '$はどんな状況であっても、周囲に正しい行いをするように働き掛けることができる。',
    opts: [
      '$は自分の利益を優先することが多く、正しい行いを促す場面は少ない',
      '$は自分では正しく行動しているが、周囲への働きかけはあまりない',
      '$はどんな状況でも誠実に行動し、周囲にも正しい行いを促せる',
      '$は高い倫理観で常に模範となり、組織全体の信頼の柱になっている' ] },
  { stmt: '$は変化への対応力とともに、その場で機転を利かせて行動を適宜修正することができる。',
    opts: [
      '$は変化に対して戸惑うことが多く、対応に時間がかかる傾向がある',
      '$は時間をかければ変化に適応できるが、機転を利かせるのは難しい',
      '$は変化への対応力があり、その場で機転を利かせて行動を修正できる',
      '$はどんな変化にも即座に対応し、新しい状況でも高いパフォーマンスを発揮できる' ] },
  { stmt: '$は自分とは考えや意見の異なる相手に対しても理解を示し、それを許容することができる。',
    opts: [
      '$は自分と異なる意見に対して、受け入れることが難しそうに見える',
      '$は異なる意見も表面上は受け入れるが、深く理解するまでには至らない',
      '$は異なる考えの相手にも理解を示し、許容する態度を持っている',
      '$は多様な価値観を積極的に取り入れ、対立をまとめる力がある' ] },
  { stmt: '$は組織の目的や目標を正しく理解した上で、その実現のために真剣に取り組むことができる。',
    opts: [
      '$は自分の役割はこなしているが、組織目標への関心はあまり高くない',
      '$は組織の目標を理解しているが、自発的な貢献は限られている',
      '$は組織の目的を正しく理解し、その実現のために真剣に取り組んでいる',
      '$は組織への強い愛着を持ち、自己利益を超えて組織の発展に貢献している' ] },
];
      ...
      <div className="scroll pad stack">
        <div className="card fade-in" style={{ borderRadius: 'var(--r-md)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-sub)' }}>{PEER_NAME}について</span>
          </div>
          <p style={{ fontSize: 13.5, fontWeight: 700, color: 'var(--text)', lineHeight: 1.75, marginBottom: 14, textWrap: 'pretty' }}>{PEER_NAME}の長所など、優れているところをフリーワードで入力してね。</p>
        ...
        <div className="card" style={{ borderRadius: 'var(--r-md)', minHeight: 170 }}>
          <div style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--text)', marginBottom: 12 }}>{PEER_NAME}の強みは</div>
          {tags.length === 0 ? (

...
function ContentTeaser({ unlocked, onOpen }) {
  if (unlocked) {
    return (
      <button onClick={onOpen} style={{ width: '100%', border: 'none', cursor: 'pointer', textAlign: 'left', background: '#fff5cc', borderRadius: 'var(--r-lg)', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ color:'var(--blue)', display:'flex' }}><FIcon name="chart" size={26} /></div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--blue-dark)' }}>レポートが見られるよ！</div>
          <div style={{ fontSize: 10.5, color: '#5a6b9a' }}>過去の受検レポートをチェック</div>
        </div>
  ...
  return (
    <div style={{ background: '#f0edea', borderRadius: 'var(--r-lg)', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-sub)' }}>過去の受検レポート</div>
        <div style={{ fontSize: 10, color: 'var(--text-sub)' }}>トリセツが完成するとレポートが見られます</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="card">
      <div style={{ fontFamily: 'var(--font-round)', fontWeight: 800, fontSize: 15, marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
        <span style={{ background: '#ffd633', padding: '1px 6px' }}>{action.title}</span>
      </div>
...
function HomeVarA({ state, comp }) {
  ...
  return (
    <div className="stack">
      <div style={{ background: '#315cfa', borderRadius: 'var(--r-lg)', padding: 18, color: '#fff', boxShadow: 'var(--shadow-blue)' }}>
...
function HomeVarC({ state, comp }) {
  ...
  return (
    <div className="stack">
      <div className="card" style={{ padding: '20px 18px', background: '#fff' }}>
            ...
            return (
              <div key={s.key} style={{ display: 'flex', gap: 14, position: 'relative', paddingBottom: isLast ? 0 : 18 }}>
                ...
                <div style={{ width: 40, height: 40, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, position: 'relative', zIndex: 2,
                  ...
                  boxShadow: isCurrent ? '0 0 0 5px rgba(255,107,94,.18)' : 'none', color: s.done ? '#fff' : 'inherit' }}>
                  {s.done ? <span style={{fontWeight:900}}>✓</span> : <FIcon name={s.emoji} size={17} />}
                </div>
